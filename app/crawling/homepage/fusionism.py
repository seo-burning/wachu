...
def sub_crawler(obj, product_source, url_obj, duplicate_check_list):
    ...
    for product_obj in product_source:
        ...
        product_link = "http://fusionism.vn"+product_link_soup.get('href')
        print(product_link)
        if product_link not in duplicate_check_list:
            ...
            description = option_json['metadescription']
            if description == None:
                description = name
...
def get_fusionism():
    ...
    for url_obj in url_list:
        i = 1
        while True:
            url = "http://fusionism.vn/collections/"+url_obj+"?page=" + str(i)
            response = obj.request_url(url)
            soup = BeautifulSoup(response.text, 'html.parser')
            product_source = soup.find_all("div", class_="product-block")
            product_sub_list, duplicate_check_list = sub_crawler(obj, product_source, url_obj,
                                                                 duplicate_check_list)
