...
def get_product_obj_from_json(store, url_obj, product_link, option_json):
        #     title / description
...
#     product price
    ...
    if product_price.find('span', class_='pro-sale'):
        ...
        discount_rate = product_price.find('span', class_='pro-sale').text.replace('-', '').replace('%', '').replace(' ','')
    else:
...
def sub_crawler(obj, product_source,url_obj, duplicate_check_list):
    ...
    for product_obj in product_source:
        ...
        product_link = "http://heyyoustudio.vn"+product_link_soup.get('href')
        print(product_link)
...
def get_heyyoustudio():
    print('operating heyyoustudio crawler')
    obj = HomepageCrawler()
    url_list = ['all']
    product_list = []
    ...
    for url_obj in url_list:
        i = 1
        while True:
            url = "http://heyyoustudio.vn/collections/"+url_obj+"?page=" + str(i)
